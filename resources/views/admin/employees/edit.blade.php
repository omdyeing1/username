                ...
                <form action="{{ route('admin.employees.update', $employee) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="payment_mode" id="payment_mode" class="form-select" required onchange="toggleSalaryInputs()">
                            ...
                            <option value="fixed" {{ old('payment_mode', $employee->payment_mode) == 'fixed' ? 'selected' : '' }}>Fixed Salary</option>
                            <option value="pcs" {{ old('payment_mode', $employee->payment_mode) == 'pcs' ? 'selected' : '' }}>Piece Rate</option>
                            {{-- <option value="trip" {{ old('payment_mode', $employee->payment_mode) == 'trip' ? 'selected' : '' }}>Trip Based</option> --}}
                        </select>
                    ...
                    <div id="fixed_input" class="mb-3" style="display: none;">
                        ...
                        <input type="number" name="fixed_salary" class="form-control" step="0.01" min="0" value="{{ old('fixed_salary', $employee->fixed_salary) }}">
                    ...
                    <div id="piece_input" class="mb-3" style="display: none;">
                        ...
                        <input type="number" name="pcs_rate" class="form-control" step="0.01" min="0" value="{{ old('pcs_rate', $employee->pcs_rate) }}">
                   ...
                   {{-- <div id="trip_input" class="mb-3" style="display: none;">
                        ...
                        <input type="number" name="trip_rate" class="form-control" step="0.01" min="0" value="{{ old('trip_rate', $employee->trip_rate) }}">
                        @error('trip_rate') <div class="text-danger small">{{ $message }}</div> @enderror
                    </div> --}}
                    
                    <h6 class="mb-3 text-muted">Account Status</h6>
                     <div class="mb-4">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="is_blocked" name="is_blocked" value="1" {{ old('is_blocked', $employee->is_blocked) ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_blocked">Block this employee (Prevent login)</label>
                        </div>
                    </div>
